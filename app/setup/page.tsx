...
const CORES_PRESET = [
  { nome: "Dourado (atual)", primaria: "#9B8559", secundaria: "#b8a07a" },
  { nome: "Azul Corporativo", primaria: "#1a4fa0", secundaria: "#3a6fc7" },
...
export default function SetupPage() {
  ...
  return (
    <main style={{ background: "#F9F8F7", minHeight: "100vh", fontFamily: "Georgia,serif" }}>
      <div style={{ background: "linear-gradient(135deg,#9B8559,#b8a07a)", padding: "30px 40px", textAlign: "center" }}>
        <h1 style={{ color: "#fff", margin: 0, fontSize: 22, letterSpacing: 2, fontWeight: "normal" }}>GERADOR DE AGENTE DE CURADORIA</h1>
        <p style={{ color: "#F6E6EA", margin: "8px 0 0", fontSize: 13 }}>Preencha o formulário e receba o config.ts pronto</p>
      </div>

      <div style={{ maxWidth: 720, margin: "0 auto", padding: "40px 20px" }}>

        <div style={{ background: "#fff", border: "1px solid #e8e2d8", borderRadius: 10, padding: 24, marginBottom: 20 }}>
          <h2 style={{ color: "#9B8559", fontSize: 16, margin: "0 0 20px", letterSpacing: 1 }}>1. QUEM VAI RECEBER</h2>
          {([
            ...
            <div key={k} style={{ marginBottom: 16 }}>
              ...
              <input style={{ width: "100%", padding: "10px 12px", border: "1px solid #ddd", borderRadius: 6, fontSize: 14, fontFamily: "Georgia,serif", boxSizing: "border-box" }} placeholder={ph} value={(form as Record<string,string>)[k]} onChange={e => set(k, e.target.value)} />
            </div>
          ))}
        </div>

        <div style={{ background: "#fff", border: "1px solid #e8e2d8", borderRadius: 10, padding: 24, marginBottom: 20 }}>
          <h2 style={{ color: "#9B8559", fontSize: 16, margin: "0 0 20px", letterSpacing: 1 }}>2. CONFIGURAÇÃO DE E-MAIL</h2>
          <div style={{ marginBottom: 16 }}>
            <label style={{ display: "block", fontSize: 13, color: "#666", marginBottom: 6 }}>E-mail de destino *</label>
            <input style={{ width: "100%", padding: "10px 12px", border: "1px solid #ddd", borderRadius: 6, fontSize: 14, fontFamily: "Georgia,serif", boxSizing: "border-box" }} placeholder="email@exemplo.com" value={form.emailDestinatario} onChange={e => set("emailDestinatario", e.target.value)} />
          </div>
          <div style={{ marginBottom: 16 }}>
            <label style={{ display: "block", fontSize: 13, color: "#666", marginBottom: 6 }}>Assunto do e-mail</label>
            <input style={{ width: "100%", padding: "10px 12px", border: "1px solid #ddd", borderRadius: 6, fontSize: 14, fontFamily: "Georgia,serif", boxSizing: "border-box" }} value={form.assunto} onChange={e => set("assunto", e.target.value)} />
          </div>
          <div>
            <label style={{ display: "block", fontSize: 13, color: "#666", marginBottom: 6 }}>Horário de envio (horário de Brasília)</label>
            <select style={{ width: "100%", padding: "10px 12px", border: "1px solid #ddd", borderRadius: 6, fontSize: 14, fontFamily: "Georgia,serif" }} value={form.horario} onChange={e => set("horario", e.target.value)}>
        ...
        <div style={{ background: "#fff", border: "1px solid #e8e2d8", borderRadius: 10, padding: 24, marginBottom: 20 }}>
          <h2 style={{ color: "#9B8559", fontSize: 16, margin: "0 0 20px", letterSpacing: 1 }}>3. COR DO E-MAIL</h2>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
            {CORES_PRESET.map((c, i) => (
              <button key={i} onClick={() => set("corPreset", String(i))} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 14px", border: form.corPreset === i ? "2px solid #333" : "2px solid #ddd", borderRadius: 20, background: "#fff", cursor: "pointer", fontSize: 13, fontFamily: "Georgia,serif" }}>
        ...
        <div style={{ background: "#fff", border: "1px solid #e8e2d8", borderRadius: 10, padding: 24, marginBottom: 20 }}>
          <h2 style={{ color: "#9B8559", fontSize: 16, margin: "0 0 8px", letterSpacing: 1 }}>4. FONTES DE CONTEÚDO (RSS)</h2>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 20 }}>
            {(Object.keys(FEEDS_EXEMPLO) as Array<keyof typeof FEEDS_EXEMPLO>).map(area => (
              <button key={area} onClick={() => usarExemplo(area)} style={{ padding: "6px 14px", background: "#f5f0e8", border: "1px solid #ddd", borderRadius: 20, cursor: "pointer", fontSize: 13, fontFamily: "Georgia,serif", color: "#9B8559" }}>
          ...
          {feeds.map((feed, i) => (
            <div key={i} style={{ background: "#fafafa", border: "1px solid #eee", borderRadius: 8, padding: 16, marginBottom: 12 }}>
              ...
              {(["url", "categoria", "fonte"] as const).map(k => (
                <input key={k} style={{ width: "100%", padding: "8px 10px", border: "1px solid #ddd", borderRadius: 6, fontSize: 13, fontFamily: "Georgia,serif", marginBottom: 8, boxSizing: "border-box" }} placeholder={k === "url" ? "https://site.com/rss.xml" : k === "categoria" ? "Ex: Tecnologia" : "Ex: TechCrunch"} value={feed[k]} onChange={e => updateFeed(i, k, e.target.value)} />
              ))}
            </div>
          ))}
          <button onClick={addFeed} style={{ width: "100%", padding: "10px", border: "2px dashed #ddd", borderRadius: 8, background: "none", cursor: "pointer", color: "#9B8559", fontSize: 14, fontFamily: "Georgia,serif" }}>+ Adicionar feed</button>
        </div>

        <div style={{ background: "#fff", border: "1px solid #e8e2d8", borderRadius: 10, padding: 24, marginBottom: 20 }}>
          <h2 style={{ color: "#9B8559", fontSize: 16, margin: "0 0 8px", letterSpacing: 1 }}>5. SEÇÕES DO E-MAIL</h2>
          <p style={{ fontSize: 13, color: "#888", margin: "0 0 12px" }}>Uma seção por linha — como o conteúdo será agrupado</p>
          <textarea style={{ width: "100%", padding: "10px 12px", border: "1px solid #ddd", borderRadius: 6, fontSize: 14, fontFamily: "Georgia,serif", boxSizing: "border-box", minHeight: 100 }} value={form.secoes} onChange={e => set("secoes", e.target.value)} />
        </div>

        <div style={{ background: "#fff", border: "1px solid #e8e2d8", borderRadius: 10, padding: 24, marginBottom: 20 }}>
          <h2 style={{ color: "#9B8559", fontSize: 16, margin: "0 0 8px", letterSpacing: 1 }}>6. FILTRO DE QUALIDADE</h2>
          <div style={{ marginBottom: 16 }}>
            <label style={{ display: "block", fontSize: 13, color: "#666", marginBottom: 6 }}>O que incluir (um por linha)</label>
            <textarea style={{ width: "100%", padding: "10px 12px", border: "1px solid #ddd", borderRadius: 6, fontSize: 14, fontFamily: "Georgia,serif", boxSizing: "border-box", minHeight: 80 }} value={form.incluir} onChange={e => set("incluir", e.target.value)} />
          </div>
          <div>
            <label style={{ display: "block", fontSize: 13, color: "#666", marginBottom: 6 }}>O que excluir (um por linha)</label>
            <textarea style={{ width: "100%", padding: "10px 12px", border: "1px solid #ddd", borderRadius: 6, fontSize: 14, fontFamily: "Georgia,serif", boxSizing: "border-box", minHeight: 80 }} value={form.excluir} onChange={e => set("excluir", e.target.value)} />
          </div>
        </div>

        <div style={{ background: "#fff", border: "1px solid #e8e2d8", borderRadius: 10, padding: 24, marginBottom: 28 }}>
          <h2 style={{ color: "#9B8559", fontSize: 16, margin: "0 0 12px", letterSpacing: 1 }}>7. IDIOMA DE SAÍDA</h2>
          <select style={{ width: "100%", padding: "10px 12px", border: "1px solid #ddd", borderRadius: 6, fontSize: 14, fontFamily: "Georgia,serif" }} value={form.idioma} onChange={e => set("idioma", e.target.value)}>
        ...
        <button onClick={gerar} style={{ width: "100%", padding: "16px", background: "linear-gradient(135deg,#9B8559,#b8a07a)", color: "#fff", border: "none", borderRadius: 10, fontSize: 16, fontFamily: "Georgia,serif", cursor: "pointer", letterSpacing: 1, marginBottom: 32 }}>
          GERAR MEU CONFIG.TS
        </button>

        {gerado && (
          <div style={{ background: "#fff", border: "1px solid #e8e2d8", borderRadius: 10, padding: 24, marginBottom: 32 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              <h2 style={{ color: "#9B8559", fontSize: 16, margin: 0, letterSpacing: 1 }}>SEU CONFIG.TS PRONTO</h2>
              <button onClick={copiar} style={{ padding: "8px 20px", background: copiado ? "#2e7d5e" : "#9B8559", color: "#fff", border: "none", borderRadius: 20, cursor: "pointer", fontSize: 13, fontFamily: "Georgia,serif" }}>
            ...
            <div style={{ background: "#f5f0e8", border: "1px solid #ddd", borderRadius: 8, padding: 20, marginTop: 20 }}>
              <h3 style={{ color: "#9B8559", fontSize: 14, margin: "0 0 12px", letterSpacing: 1 }}>PRÓXIMOS PASSOS</h3>
