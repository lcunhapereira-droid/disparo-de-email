...
const App: React.FC = () => {
  ...
  const handleInspirationSubmit = useCallback(async () => {
    ...
    try {
      const result = await generateInspirationImage(inspirePrompt);
      if (result) {
        setGeneratedImages([{ 
          src: `data:image/jpeg;base64,${result}`, 
          title: 'Sua Inspiração' 
        }]);
      } else {
         setError('Não foi possível gerar uma imagem de inspiração. Tente um prompt diferente.');
      }
    } catch (err: any) {
  ...
  const renderTransformTab = () => (
    <div className="w-full space-y-6">
      ...
      <button
        ...
        disabled={isLoading || !imagePayload || !transformPrompt}
  ...
  const renderInspireTab = () => (
     <div className="w-full space-y-6">
       ...
       <textarea
        ...
        placeholder="Descreva um look ou estilo de beleza... ex: 'Um penteado futurista cromado' ou 'Maquilhagem etérea de fada'"
        className={commonTextAreaClasses}
        rows={3}
      />
      <button
        onClick={handleInspirationSubmit}
        disabled={isLoading || !inspirePrompt}
        className={commonButtonClasses}
      >
        <SparklesIcon />
        Gerar Inspiração
        ...
        <div className="bg-[#111] rounded-lg shadow-2xl shadow-black/30 border border-brand-gold/20 p-6 sm:p-8 space-y-8">
          <div className="flex justify-center border-b border-brand-light/20 pb-4">
            <div className="flex space-x-4">
              <TabButton isActive={activeTab === 'transform'} onClick={() => setActiveTab('transform')}>
                <CameraIcon /> Transformação de Beleza
              </TabButton>
              <TabButton isActive={activeTab === 'inspire'} onClick={() => setActiveTab('inspire')}>
                <SparklesIcon /> Gerador de Inspiração
              </TabButton>
            </div>
          </div>
          
          <div className="flex flex-col items-center">
            {activeTab === 'transform' ? renderTransformTab() : renderInspireTab()}
          </div>
